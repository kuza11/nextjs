...
export default async function RWPersonById(
  ...
  if (req.method === "GET") {
    ...
    const data = await writeDB({table: req.query.table, id: req.query.id}, {username: req.body.username, password: req.body.password, title: req.body.title, description: req.body.description});
    if(data.changes == 0) res.status(404).json({message: "element does not exist"});
    else res.status(200).json({message: "success"});
  } else if (req.method == "DELETE"){
    const data = await deleteDB({table: req.query.table, id: req.query.id});
